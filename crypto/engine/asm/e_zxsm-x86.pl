...
%ZXSM_PREFETCH=(ecb=>128, cbc=>64);	# prefetch errata
$ZXSM_CHUNK=512;	# Must be a power of 2 larger than 16
...
&function_begin_B("zxsm_capability");
	&push	("ebx");
	&pushf	();
	&pop	("eax");
	&mov	("ecx","eax");
...
&set_label("noluck");
	&pop	("ebx");
	&ret	();
&function_end_B("zxsm_capability")

&function_begin_B("zxsm_key_bswap");
	&mov	("edx",&wparam(0));
	&mov	("ecx",&DWP(240,"edx"));
&set_label("bswap_loop");
	...
	&ret	();
&function_end_B("zxsm_key_bswap");

# This is heuristic key context tracing. At first one
# believes that one should use atomic swap instructions,
# but it's not actually necessary. Point is that if
# zxsm_saved_context was changed by another thread
# after we've read it and before we compare it with ctx,
# our key *shall* be reloaded upon thread context switch
# and we are therefore set in either case...
&static_label("zxsm_saved_context");

&function_begin_B("zxsm_verify_context");
	&mov	($ctx,&wparam(0));
	&lea	("eax",($::win32 or $::coff) ? &DWP(&label("zxsm_saved_context")) :
		       &DWP(&label("zxsm_saved_context")."-".&label("verify_pic_point")));
	&pushf	();
	&call	("_zxsm_verify_ctx");
&set_label("verify_pic_point");
	&lea	("esp",&DWP(4,"esp"));
	&ret	();
&function_end_B("zxsm_verify_context");

&function_begin_B("_zxsm_verify_ctx");
	&add	("eax",&DWP(0,"esp")) if(!($::win32 or $::coff));# &zxsm_saved_context
...
&set_label("verified");
	&mov	(&DWP(0,"eax"),$ctx);
	&ret	();
&function_end_B("_zxsm_verify_ctx");

&function_begin_B("zxsm_reload_key");
	&pushf	();
	&popf	();
	&ret	();
&function_end_B("zxsm_reload_key");

&function_begin_B("zxsm_aes_block");
	&push	("edi");
	&push	("esi");
	&push	("ebx");
...
&function_begin("zxsm_${mode}_encrypt");
	...
	&jnz	(&label("${mode}_abort"));
	&lea	("eax",($::win32 or $::coff) ? &DWP(&label("zxsm_saved_context")) :
		       &DWP(&label("zxsm_saved_context")."-".&label("${mode}_pic_point")));
	&pushf	();
	&cld	();
	&call	("_zxsm_verify_ctx");
&set_label("${mode}_pic_point");
	...
	&mov	($chunk,$ZXSM_CHUNK);
	&not	("eax");		# out_misaligned?-1:0
	&lea	("ebp",&DWP(-24,"esp"));
	&cmp	($len,$chunk);
	&cmovc	($chunk,$len);		# chunk=len>ZXSM_CHUNK?ZXSM_CHUNK:len
	&and	("eax",$chunk);		# out_misaligned?chunk:0
	&mov	($chunk,$len);
	&neg	("eax");
	&and	($chunk,$ZXSM_CHUNK-1);	# chunk=len%ZXSM_CHUNK
	&lea	("esp",&DWP(0,"eax","ebp"));	# alloca
	&mov	("eax",$ZXSM_CHUNK);
	&cmovz	($chunk,"eax");			# chunk=chunk?:ZXSM_CHUNK
	&mov	("eax","ebp");
	&and	("ebp",-16);
	&and	("esp",-16);
	&mov	(&DWP(16,"ebp"),"eax");
	...
	&cmp	("eax",$ZXSM_PREFETCH{$mode});
	&mov	("eax",-$ZXSM_PREFETCH{$mode});
...
&set_label("${mode}_aligned",16);
	...
	&xor	("eax","eax");
	&cmp	("ebp",$ZXSM_PREFETCH{$mode});
	&mov	("ebp",$ZXSM_PREFETCH{$mode}-1);
	&cmovae	("ebp","eax");
...
&function_end("zxsm_${mode}_encrypt");
}
...
&function_begin_B("zxsm_sha1_oneshot");
	&push	("edi");
	&push	("esi");
	&xor	("eax","eax");
	&mov	("edi",&wparam(0));
	...
	&pop	("edi");
	&ret	();
&function_end_B("zxsm_sha1_oneshot");

&function_begin_B("zxsm_sha1_blocks");
	&push	("edi");
	&push	("esi");
	&mov	("edi",&wparam(0));
	&mov	("esi",&wparam(1));
	...
	&pop	("edi");
	&ret	();
&function_end_B("zxsm_sha1_blocks");

&function_begin_B("zxsm_sha256_oneshot");
	&push	("edi");
	&push	("esi");
	&xor	("eax","eax");
	&mov	("edi",&wparam(0));
	...
	&pop	("edi");
	&ret	();
&function_end_B("zxsm_sha256_oneshot");

&function_begin_B("zxsm_sha256_blocks");
	&push	("edi");
	&push	("esi");
	&mov	("edi",&wparam(0));
	&mov	("esi",&wparam(1));
	...
	&pop	("edi");
	&ret	();
&function_end_B("zxsm_sha256_blocks");

&function_begin_B("zxsm_sha512_blocks");
	&push	("edi");
	&push	("esi");
	&mov	("edi",&wparam(0));
	&mov	("esi",&wparam(1));
	...
	&pop	("edi");
	&ret	();
&function_end_B("zxsm_sha512_blocks");

&asciz	("Shanghai ZXSM x86 module");
&align	(16);
...
&set_label("zxsm_saved_context",4);
&data_word(0);
